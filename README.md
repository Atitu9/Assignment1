...
* **Step 1:** Structured text using headings (`<h1>`-`<h3>`) and paragraph (`<p>`).
* **Step 2:** Created ordered (`<ol>`) and unordered (`<ul>`) lists.
* **Step 3:** Inserted image (`<img>`) and external links (`<a>`).
...
* **Step 16:** Grouped structure using `<div>` elements (`site-header`, `main-content`).
